    ...
    protected Node split(final RectNd t) {
        ...
        for (int d = 0; d < nD; d++) {
            ...
            for (int j = 1; j < size; j++) {
                if (r[rIndex[d][MIN][MIN]].getMin().getCoord(d) > (r[j].getMin().getCoord(d))) {
                    rIndex[d][MIN][MIN] = j;
                }

                if (r[rIndex[d][MIN][MAX]].getMin().getCoord(d) < (r[j].getMin().getCoord(d))) {
                    rIndex[d][MIN][MAX] = j;
                }

                if (r[rIndex[d][MAX][MIN]].getMax().getCoord(d) > (r[j].getMax().getCoord(d))) {
                    rIndex[d][MAX][MIN] = j;
                }

                if (r[rIndex[d][MAX][MAX]].getMax().getCoord(d) < (r[j].getMax().getCoord(d))) {
                    rIndex[d][MAX][MAX] = j;
                }
            }
